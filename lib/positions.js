...
module.exports = class Positions {
    static deserialize(params, state){
        return new Positions(params, state);
    }

    serialize(){
        return {
            deserializer: 'Positions',
            uri: this.getURI(),
    ...
    constructor({manager, omnibar}, state){
        ...
        this.disposables.add(via.config.observe('positions.hideZeroValues', this.update.bind(this)));
        ...
        this.disposables.add(via.commands.add(this.element, 'positions:update-positions', this.updatePositions.bind(this)));
    }
